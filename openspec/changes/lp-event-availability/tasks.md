...
- [ ] 5.3 dev でローカル動作確認（`pnpm dev:lp --port 5275`）: Loading / Empty / Error / Success の4状態、募集中・満員・無制限の各イベント表示（一覧＋HERO 下ストリップ両方）、残席取得失敗時のグレースフル劣化、モバイル表示、コントラスト（AA）を探索的に確認

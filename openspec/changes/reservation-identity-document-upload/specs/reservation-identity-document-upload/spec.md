...
- **WHEN** 認証済 + プロフィール完成 + `hasIdentityDocument === false` のユーザーが `/` にアクセス
...
#### Scenario: 書類提出済 + `/signup/identity` 直リン
- **WHEN** 認証済 + プロフィール完成 + 書類提出済のユーザーが `/signup/identity` に直接アクセス
- **THEN** `/` (ホーム) にリダイレクトされる

### Requirement: AuthSession に `hasIdentityDocument` 派生プロパティが存在する

`useAuthSession` の戻り値型 `AuthSession` に `hasIdentityDocument: ComputedRef<boolean>` を MUST 含める。`evaluate()` 実行時に `members` 取得と並行 (Promise.all 等) で `select id from identity_documents where member_id = ? limit 1` を実行し、行が 1 件以上あれば `true`、0 件なら `false` を保持する SHALL。`refresh()` で再評価される。

#### Scenario: 未提出会員の取得
- **WHEN** 書類を 1 件も提出していない会員が `useAuthSession.ready()` を呼ぶ
- **THEN** `hasIdentityDocument.value === false` となる

#### Scenario: 1 件以上提出済会員の取得
- **WHEN** `identity_documents` に 1 行以上ある会員が `useAuthSession.ready()` を呼ぶ
- **THEN** `hasIdentityDocument.value === true` となる (status が pending/approved/rejected いずれでも true)

#### Scenario: アップロード成功後の refresh
- **WHEN** 書類アップロード成功直後に `useAuthSession.refresh()` が呼ばれる
- **THEN** `hasIdentityDocument` が `true` に更新される

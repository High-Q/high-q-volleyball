...
イベントカードは `vol` が非 NULL のとき回号を `vol.NN` 形式で表示する SHALL。`vol` が NULL（未採番）のときは回号を表示しない MUST。表示色・書体は HQ デザイントークン（`var(--hq-*)`）経由とする MUST。
...
- **WHEN** `vol = 74` のイベントがカードに描画される
- **THEN** カードに `vol.74` が表示される

#### Scenario: 未採番イベントは回号を表示しない
- **WHEN** `vol = null` のイベントがカードに描画される
- **THEN** カードに回号は表示されない

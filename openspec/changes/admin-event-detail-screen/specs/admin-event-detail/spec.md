...
1. **名前**: アバター（先頭文字の丸チップ）+ `members.display_name` + 初回バッジ（`is_first_time === true` の場合のみ）
...
- **検索**: `members.display_name` または `members.email` への部分一致（ILIKE）
...
- **THEN** URL が `/events/<id>?q=%E7%94%B0%E4%B8%AD` になり、display_name または email に「田中」を含む行のみ表示される

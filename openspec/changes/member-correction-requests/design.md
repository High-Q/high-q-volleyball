...
### 決定 2: バナーは `/events`（home）のみに表示し、dismissal 機能なし

- 表示場所: 認証済ユーザーが最初に到達する `/events` ページ上部
- 「閉じる」UI は付けない。修正完了で自動消滅する以外には消えない
- 複数件あれば縦に積み上げ表示（高々 3〜4 件を想定。admin が無闇に作らない運用前提）

**Why:**

- dismissal を入れると「会員ごとの閉じた状態」を localStorage / DB に持つ必要があり複雑化
- 軽量 MVP として「目に入る = 修正動線が常時提示される」をシンプルに実現
- 修正完了で自動消えるので「邪魔だが消せない」状態は実害的に起きない

`/profile` ページや他の認証済ページにはバナーを置かない（home 1 箇所のみ）。

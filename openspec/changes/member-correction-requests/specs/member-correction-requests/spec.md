...
- `field` (string) — 修正対象の属性名。許容値は `'last_name' | 'first_name' | 'birthday' | 'phone' | 'experience_level' | 'nickname'` のみ SHALL
...
- **WHEN** 同会員に `birthday` / `last_name` / `first_name` の 3 件の修正依頼が同時存在
- **THEN** 全 3 エントリが配列に保持され、会員サイトのバナーで 3 件が縦に積み上げ表示される
...
|---|---|
| `updateMyName(lastName, firstName)` | `last_name` AND `first_name` のエントリ両方 |
...
#### Scenario: 氏名 update で姓・名 両エントリが消える
- **WHEN** 会員に `field=last_name` と `field=first_name` の 2 件の未対応依頼があり、`updateMyName` で姓・名 両方を一度に更新
- **THEN** 両エントリが配列から削除される

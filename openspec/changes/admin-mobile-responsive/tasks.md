...
- [ ] 5.1 `LoginPage` をモバイル縦積み（ブランド帯を上部に圧縮・装飾コピー非表示・フォーム全幅）、デスクトップ 2 カラム維持
- [ ] 5.2 `MfaChallengePage` / `MfaSetupPage` / `AuthCallbackPage` がモバイル幅で中央寄せ・横スクロール無しで収まることを確認
- [ ] 5.3 既存のエラー表示（widget の Error 状態等）がモバイル幅で折り返し・破綻しないことを確認

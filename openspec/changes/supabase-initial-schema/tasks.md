...
- 完了: 35 / 42 タスク（Task 1〜7 完了。残: Task 8 動作確認・PR 作成）
...
- [x] 7.2 `docs/04-システム設計/01-DB設計/02-物理設計/物理設計.md` に PostgreSQL 固有の決定（型・インデックス・トリガー・RLS）を記載（Apply 中、SQL Migration と並行で更新）
- [x] 7.3 `docs/06-品質・セキュリティ/03-アクセス制御・認可設計.md` に RLS ポリシー詳細を記載
- [x] 7.4 「管理者ユーザー作成手順（SQL 直書き）」を `docs/06-品質・セキュリティ/03-アクセス制御・認可設計.md §7` に記載（README は vue-cli 時代の古い内容で全面刷新が必要、本 PR スコープ外として別 issue へ）

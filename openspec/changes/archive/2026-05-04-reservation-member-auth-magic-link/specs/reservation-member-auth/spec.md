...
`apps/reservation` は `/auth/callback` ルートを SHALL 提供する。Supabase クライアントが `detectSessionInUrl: true` で構成されているため、ページマウント時に URL hash 内のトークンが消化されてセッションが確立する。`AuthCallbackPage.vue` は session 確立を待ち、`isProfileComplete` 判定の結果に応じてリダイレクトする MUST（D6 改訂により localStorage payload 適用ロジックは削除）。
...
`apps/reservation` は `signInWithOtp` 成功後に **送信先メールアドレス + 再送ボタン + 別メールで送り直すリンク** を持つ画面を SHALL 表示する。`LinkSentPage.vue` 単独ルートでも、`LoginPage` / `SignupPage` 内の Success state でも実装可（実装は `apps/reservation/src/pages/LinkSentPage.vue` 単独ルートを採用）。

#### Scenario: 送信先メールアドレス表示
- **WHEN** `signInWithOtp` 成功で LinkSent 画面に遷移
- **THEN** 「<入力メール> 宛にログインリンクを送信しました。メール内のリンクから続行してください。」が表示される

#### Scenario: メール再送
- **WHEN** ユーザーが「メールを再送する」ボタンを押す
- **THEN** 同じメールアドレスで `signInWithOtp` が再実行され、Loading 状態を経て Success に戻る（または rate-limit エラーで Error 状態）
...
#### Scenario: 別メールで送り直す
- **WHEN** ユーザーが「別のメールアドレスで送り直す」リンクを押す
- **THEN** Login 画面（または Signup 画面）に戻り、入力欄がクリアされる

### Requirement: 4 状態 UI

`/signup` / `/login` / `/auth/link-sent` / `/auth/callback` の各ページは Empty / Loading / Error / Success の 4 状態を持ち、各状態で表示要素が明確に切り替わる SHALL。

#### Scenario: SignupPage Empty 状態
- **WHEN** `/signup` に初回アクセスする
- **THEN** 全フィールド入力欄 + 同意チェックボックス + 「無料で会員登録する」CTA が表示され、CTA は同意 OFF のため disabled
...
#### Scenario: 未認証で `/` にアクセス
- **WHEN** 未認証ユーザーが `/` にアクセス
- **THEN** `/` のページ（HomePlaceholder / 後続のイベント一覧）が描画される（リダイレクトされない）

#### Scenario: 未認証で予約ルートにアクセス（将来）
- **WHEN** 未認証ユーザーが将来実装される予約ルート（`meta.public` が無いルート）にアクセス
- **THEN** `/login` にリダイレクトされる
...
#### Scenario: プロフィール未完成で `/signup` にアクセス
- **WHEN** 認証済み + `isProfileComplete === false` のユーザーが `/signup` にアクセス
- **THEN** `/signup/profile` にリダイレクトされる（メール送信は不要、既に session があるため）

### Requirement: 中途離脱ユーザーの 48h cleanup（後続 Issue で実装）

本 change 内では中途離脱ユーザー（`auth.users` unconfirmed / `members.profile.signup_completed != true`）を **削除しない** が、後続 Issue で実装される cleanup ジョブが本 change の `signup_completed` フラグを判定根拠に SHALL 利用する。本 Requirement は cleanup ジョブの**判定契約**を本 change で固定するためのもので、cleanup の実装自体は本 change のスコープ外。

#### Scenario: cleanup ジョブの判定契約
- **WHEN** 後続 Issue の cleanup ジョブが日次実行される
...
- **WHEN** プロフィール完成済み（`profile.signup_completed = true`）のユーザー
...
- **WHEN** `/auth/callback` で payload 適用後に `refresh()` を呼ぶ
...
#### Scenario: 同意 ON で payload 保存
- **WHEN** 同意チェックボックスを ON にして CTA を押す
- **THEN** payload に `terms_agreed_at: '<ISO8601>'` が含まれ、callback で `members.profile.terms_agreed_at` に保存される（同時に `signup_completed: true` も保存される）
...
`apps/reservation` のコード全体を grep して、`'owner@high-q.club'` などオーナー email リテラルが認証判定ロジックに登場 SHALL NOT する（テスト fixture や docs を除く）。会員ロールの判定は「session があり member 行が存在する」ことで十分。admin 判定は本 change の範囲外（reservation サイトでは admin 機能は提供しないため）。

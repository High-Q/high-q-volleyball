...
- [ ] 7.3 dev 環境 (許可リスト宛のみ送信) で手動 E2E: 既存予約を編集し、編集後の同伴者数 / 連絡事項 / 合計金額が反映されたメールが許可リスト宛に届くこと、件名・本文冒頭が「予約完了」と混同されない文言になっていることを確認する
- [ ] 7.4 dev 環境で「期限切れ (開催当日 0:00 JST 以降) 編集」「RLS 違反相当の他会員予約への編集試行」が走った場合にメールが送信されないことをログで確認する

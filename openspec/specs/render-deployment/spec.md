...
Render を Static Site ホスティングとして採用し、`render.yaml` を Blueprint mode の真実の源として運用する。LP は本番デプロイ済、admin / reservation は #139 / #140 で順次追加予定。dev / prd 切替の `envVars` 構造、PR Preview 自動生成、未完成アプリの商用公開禁止ガバナンスなど、安全なデプロイ運用に必要な要件を定義する。
## Requirements

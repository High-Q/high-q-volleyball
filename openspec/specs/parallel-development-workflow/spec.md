# parallel-development-workflow Specification

## Purpose

2 並列 worktree 開発を「誰でも・別セッションでも・別端末でも同じ手順で再現できる」状態にする運用規約。`git worktree` の同一ブランチ二重 checkout 不可制約、dev Supabase 1 個制約、OpenSpec change の merge 衝突制約を、スクリプト / Skill / ガイドへ機械的に固定する。

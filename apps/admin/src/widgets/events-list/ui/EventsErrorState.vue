...
import type { FetchErrorCode } from "@/entities/event";

/**
 * Events 一覧の Error 状態。エラーコード別に文言を出し分け、role="alert" を付与。
 *
 * 関連: openspec/changes/admin-events-list-screen/specs/admin-events-list/spec.md
 *       openspec/changes/admin-events-list-screen/design.md (§3 異常系一覧)
 */
...
const errCode = computed(() => `ERR · supabase / events.list · ${props.errorCode}`);
</script>

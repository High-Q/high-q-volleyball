...
erDiagram
    ...
    venues {
        ...
        text meeting_point "集合場所 (会員サイト詳細表示用、default '現地集合')"
...
| id | 会場識別子 | × | PK | UUID v4 |
| name | 会場名 | × | UK | 一意（重複防止） |
| address | 住所 | ○ | - | 内部用住所（admin 参照のみ。会員サイト UI には表示しない） |
| default_fee | 標準参加費 | ○ | - | 円。NULL は都度決定 |
| access_note | アクセス情報 | ○ | - | 駅徒歩・撤収時間など運用注意（admin 参照のみ） |
| map_url | 地図 URL | ○ | - | Google Maps 等 |
| meeting_point | 集合場所 | × | - | 会員サイトのイベント詳細画面に表示。default `'現地集合'`。#90 で追加 |
...
- **会員サイトでは `address` を表示せず、代わりに `meeting_point` を表示**（#90 PR #202 の動作確認フィードバック）。`address` 列は admin 内部参照のみで MVP1 期間中に廃止する判断は保留

**初期 seed (5 行)**:

| name | address | meeting_point | default_fee | is_primary |
|---|---|---|---|---|
| 亀戸スポーツセンター | 江東区亀戸 8-22-1 | 現地集合 | 1000 | false |
| 東砂スポーツセンター | 江東区東砂 4-24-1 | 現地集合 | 1000 | false |
| 深川スポーツセンター | 江東区越中島 1-2-18 | 現地集合 | 1000 | false |
| 深川北スポーツセンター | 江東区平野 3-2-20 | 現地集合 | 1000 | false |
| 有明会場 | 江東区有明 1-8-14 | 有明テニスの森駅 | 500 | **true** |

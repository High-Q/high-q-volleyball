...
const Features = () => {
  const items = [
    { n: '01', kicker: 'BEGINNER FRIENDLY', jp: '初心者でも、本当に大丈夫。', body: '参加者の 2〜3割 は初心者です。基礎練習（パス・スパイク）では、経験者がしっかりレクチャーします。', photo: '練習・パス' },
    { n: '02', kicker: 'CALM VIBE', jp: '落ち着いた、大人の集まり。', body: '20〜30代の社会人が中心。ノリで圧倒することはありません。お酒が苦手な人も歓迎です。', photo: 'after game · izakaya' },
    { n: '03', kicker: 'NO PRESSURE', jp: '無理しない、月1〜2回。', body: '土日祝の日中または夜間に、月1〜2回。来れる時だけ参加でOK。年に1回だけ来る人もいます。', photo: 'gym · evening' },
  ];
  return (
    <section style={{ background: HQ.paperWarm, paddingBottom: 24 }}>
      <div style={{ padding: '72px 28px 36px' }}>
        <SectionTitle kicker="Why High Q" jp={<>3つの<br />ちょうどよさ。</>} en="What makes us, us." />
      </div>
      {items.map((f) => (
        <div key={f.n} style={{ marginBottom: 48 }}>
          <Photo label={f.photo} h={280} />
          <div style={{ padding: '20px 28px 0' }}>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 12, marginBottom: 10 }}>
              <span style={{ fontFamily: HQ.jpSerif, fontSize: 24, fontWeight: 600, color: HQ.accent, lineHeight: 1 }}>
                {f.n}
              </span>
              <Kicker>{f.kicker}</Kicker>
            </div>
            <h3 style={{ fontFamily: HQ.jpSerif, fontSize: 22, lineHeight: 1.5, fontWeight: 500, margin: '0 0 12px', letterSpacing: '0.02em' }}>
              {f.jp}
            </h3>
            <p style={{ fontSize: 13.5, lineHeight: 2.0, color: HQ.inkSoft, margin: 0 }}>{f.body}</p>
          </div>
        </div>
      ))}
    </section>
...
const GallerySNS = () => (
  <section style={{ padding: '72px 0 48px', background: HQ.paperWarm }}>
    ...
    <div style={{ padding: '24px 28px 0', display: 'grid', gap: 10 }}>
      <Button primary={false} full arrow>Instagram @highq_volley</Button>
...
const Footer = () => (
  <footer style={{ background: HQ.ink, color: HQ.paper, padding: '40px 28px 24px' }}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px 16px', fontSize: 12, marginBottom: 24 }}>
      {['About', 'Events', 'Member voices', 'FAQ', 'LINE OpenChat', 'Instagram', 'X (Twitter)', 'Contact'].map((l) => (
...
// ============================================================
const HighQLP = () => {
  const [mode, setMode] = useStateLP('first');

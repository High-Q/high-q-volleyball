...
| admin (`apps/admin`) | MVP1 機能完成、Render 未デプロイ (#139 待ち) | Phase 1 移行中 |
...
| 環境変数 | `VITE_SUPABASE_URL` / `VITE_SUPABASE_PUBLISHABLE_KEY` は dev / prd でそれぞれ別値。`.env.local` は dev 値、Render Dashboard 設定は #139/#140 で実施予定 | Phase 1 移行中 |

**重要**: admin / reservation は本番公開していない（Render Service 未追加）。Supabase は dev/prd 分離完了。LP は本番運用中（Supabase 不使用）。
...
4. **Render Service 追加 + env var 設定** ⏳ **#139 / #140 で実施予定**
   - `render.yaml` に admin / reservation を services 配列に追加（雛形コメントは #184 で dev/prd 切替構造に整備済）
   ...
   - Service 追加後、prd の Auth → Redirect URLs を本番ドメインに更新

...
| reservation (`apps/reservation`) | MVP1 機能完成、Render 未デプロイ (#140 待ち) | Phase 1 移行中 |
| Supabase | **dev / prd の 2 プロジェクト分離済 (#184 で実施)**。`high-q-dev` (継続) + `high-q-prd` (空 DB に migration 全件適用済) | **Phase 1 (一部完了)** |
| 環境変数 | `VITE_SUPABASE_URL` / `VITE_SUPABASE_PUBLISHABLE_KEY` は dev / prd でそれぞれ別値。`.env.local` は dev 値、admin の Render Dashboard 設定は #139 で完了。reservation は #140 で実施予定 | Phase 1 移行中 |

**重要**: admin は本番公開済 (Render Service `high-q-admin`)。reservation は未公開。Supabase は dev/prd 分離完了。LP は本番運用中（Supabase 不使用）。
...
4. **Render Service 追加 + env var 設定**
   ...
   - reservation: ⏳ **#140 で実施予定**。雛形コメントは `render.yaml` 末尾に dev/prd 切替構造で残置（#184 で整備、#139 でも更新）
...
現状 `render.yaml` の `services` 配列に登録されているのは **LP (`apps/lp`) と admin (`apps/admin`)**。Render は services に登録されたサービス用にしか PR Preview 環境を生成しない。
...
| `apps/admin` を変更 | ✅ admin の Preview URL が自動生成される（#139 で services 追加） |
| `apps/reservation` のみを変更 | ❌ **Preview は生成されない** (services に reservation 未登録) |
| `packages/*` / `supabase/migrations/*` のみ変更 | ❌ Preview は生成されない |
| `apps/lp` + `apps/admin` 同時変更 | ✅ LP / admin 両方の Preview が生成される |

### Apply 完了時の翔太郎くん向け報告ルール (Claude 厳守)

Apply 完了後の最終確認報告で「Render Preview で確認をお願いします」と書く際は、**変更範囲が `apps/lp` または `apps/admin` を含む場合**にする。`apps/reservation` のみの変更時は次のように報告する:

> ⚠️ 本 PR は `apps/reservation` のみの変更のため、Render PR Preview は生成されません (services に reservation 未登録、#140 で追加予定)。
> 動作確認はローカルで `pnpm --filter @high-q/reservation dev` を実行してください。

これを明記しないと「Render Preview に出てこない」と翔太郎くんが混乱するため、**Apply 後の毎回の報告**で変更範囲を判定して機械的に出し分ける。

### 解消タイミング

- #139 (admin デプロイ追加) ✅ マージ済 → admin の PR Preview が動く
- #140 (reservation デプロイ追加) マージ後 → reservation の PR Preview が動くようになる

reservation 完了までは本制約が継続する。
